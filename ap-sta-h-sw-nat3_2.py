...
#from mininet.link import TCLink#added for linkstate
#NAT
...
def main():
    #net = Mininet(link=TCLink) #added TCLink for wired
    net = Mininet() #added TCLink for wired
...
##    wired = {'bw':10, 'delay':'5ms', 'loss':1, "use_htb":True}
##    wireless = {'bw':10, 'delay':'5ms', 'loss':20, "use_htb":True}
...
##    sta0 = net.addStation('sta0', ip="10.1.1.5", **hostConfig)
    sta0 = net.addStation('sta0', mac='00:00:00:00:00:10', ip='192.168.1.100/24',
...
##    sta1 = net.addStation('sta1', ip="10.1.1.6", **hostConfig)
    sta1 = net.addStation('sta1', mac='00:00:00:00:00:11', ip='192.168.1.101/24',
                          defaultRoute='via 192.168.1.1')
...
##    sta2 = net.addStation('sta2', ip="10.1.1.7", **hostConfig)
    sta2 = net.addStation('sta2', mac='00:00:00:00:00:12', ip='192.168.1.102/24',
                    defaultRoute='via 192.168.1.1')    
...
##    sta3 = net.addStation('sta3', ip="10.1.1.8", **hostConfig)
    sta3 = net.addStation('sta3', mac='00:00:00:00:00:20', ip='192.168.2.100/24',
                    defaultRoute='via 192.168.2.1')
    mininet.ns3.setMobilityModel(sta3, None)
    mininet.ns3.setPosition(sta3, 10, 10, 0)
    
    sta4 = net.addStation('sta4', mac='00:00:00:00:00:21', ip='192.168.2.101/24',
                    defaultRoute='via 192.168.2.1')
    mininet.ns3.setMobilityModel(sta4, None)
    mininet.ns3.setPosition(sta4, 10, 10, 5)
    
    sta5 = net.addStation('sta5', mac='00:00:00:00:00:22', ip='192.168.2.102/24',
                    defaultRoute='via 192.168.2.1')
